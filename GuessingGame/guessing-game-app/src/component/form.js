...
const Form = (props) => {
    ...
    return(
        ...
        <input type="submit" id="startButton" value="Start the Game" onClick={(event) => startTheGame(event, props.max)} /> 
        <label>Your guess goes here</label>
        <input type="text" id="theGuess" placeholder="Your guess here" onChange={actualGuesses}/> 
        <input type="submit" id="submitButton" value="Submit" onClick={(event) => submitButton(event)} />
        </form>
        <p id="totalGuesses"></p>
        </>
    )
